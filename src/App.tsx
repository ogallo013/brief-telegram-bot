...
export function App() {
  ...
  const [activeTab, setActiveTab] = useState<'stream' | 'tea' | 'companion' | 'journeys' | 'health' | 'inbox' | 'pursuits' | 'today' | 'sources' | 'group' | 'arena' | 'quests'>('stream');
      ...
      <header className="sticky top-0 z-40 bg-[#09150E]/95 backdrop-blur-xl border-b border-[#1E3A2A] transition-all">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3">
          ...
          <div className="flex items-center gap-6 mt-3 pt-2 border-t border-[#1E3A2A] text-xs font-bold overflow-x-auto no-scrollbar">
            ...
            <button onClick={() => setActiveTab('arena')} className={`pb-1 border-b-2 cursor-pointer ${activeTab === 'arena' ? 'text-[#00FF42] border-[#00FF42]' : 'text-[#8DCF74] border-transparent'}`}>Arena</button>
            <button onClick={() => setActiveTab('quests')} className={`pb-1 border-b-2 cursor-pointer ${activeTab === 'quests' ? 'text-[#00FF42] border-[#00FF42]' : 'text-[#8DCF74] border-transparent'}`}>Quests{openQuests.length > 0 ? ` (${openQuests.length})` : ''}</button>
                  ...
                  return (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        {/* A named provider, always. */}
                        <p className="text-[9px] font-mono text-[#5C6B52] mt-0.5">
                          {r.providerName} - KES {r.valueKes.toLocaleString()} -{' '}
                          {r.remaining > 0 ? `${r.remaining} left` : 'none left'}
                        </p>
                        {!gate.allowed && (
                          <p className="text-[10px] text-[#C9A227] mt-1">
                            {gate.reason}
          ...
          <div className="max-w-3xl mx-auto px-4 py-6 space-y-5">
            ...
            {arenaView === 'home' && (
              <div className="flex flex-wrap gap-1.5">
                {([
                  ['lobby', 'Lobby'],
                  ['available', `Available Now (${availableNow.length})`],
